...
import sys
DATA_PATH = '/home/sahruday/catkin_ws/src/Obstacle_Avoidance/Data'
MODULES_PATH = '/home/sahruday/catkin_ws/src/Obstacle_Avoidance/Scripts'
sys.path.insert(0, MODULES_PATH)

from QLearning import *
from Lidar_Scan_Preprocessing import *
                ...
                if episode > MAX_EPISODES:
                    ...
                    if crash or ep_steps >= MAX_STEPS_PER_EPISODE:
                        ...
                        if crash:
                            ...
                            ( x_crash , y_crash ) = Get_Position(odomMsg)
                            theta_crash = degrees(Get_Rotation(odomMsg))
                        ...
                        if not robot_in_pos:
                            ...
                            # init pos
                            if RANDOM_INIT_POS:
                                ( x_init , y_init , theta_init ) = Set_Random_Robot_Position(setPosPub)
                            else:
                                ( x_init , y_init , theta_init ) = Set_robot_Position(setPosPub, X_INIT, Y_INIT, THETA_INIT)

                            odomMsg = rospy.wait_for_message('/odom', Odometry)
                            ( x , y ) = Get_Position(odomMsg)
                            theta = degrees(Get_Rotation(odomMsg))
                            # check init pos
                        ...
                        # First acion
                            ...
                            ( lidar, angles ) = LidarScan_to_array(msgScan)
                            ( state_ind, x1, x2 ,x3 ,x4 ) = LaserScan_Discretization(state_space, lidar)
                            crash = checkCrash(lidar)

                            ( action, status_strat ) = epsiloGreedyExploration(Q_table, state_ind, actions, EPSILON)
                            ...
                            if not (status_strat == 'epsiloGreedyExploration => OK'):
                                print('\r\n', status_strat, '\r\n')
                                log_sim_info.write('\r\n'+status_strat+'\r\n')
